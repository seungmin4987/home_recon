...
import json  # ⭐ 변경: 평면 정보 저장을 위해 추가
...
# ============================================================
# 🌐 서버 전송 함수 (GLB 저장 + 평면 정렬 + 시각화)
#   → 세그를 어느 이미지에서 했는지 seg_image_path로 함께 전달
# ============================================================
def upload_to_server(images, seg_mask, seg_image_path):
    SERVER_URL = "https://untribal-memorisingly-joanne.ngrok-free.dev/upload"  # 필요하면 수정
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # ⭐ 변경: 평면 방정식/메타데이터를 JSON 파일로 저장
            meta_path = os.path.join(save_dir, "received_model_meta.json")
            meta_data = {
                "glb_path": out_path,
                "seg_image_name": seg_name,
                "plane_equation": plane_eq if plane_eq is not None else None,
            ...
            # 🔹 GLB 로드 & 바닥 평면을 z=0으로 정렬 + z=0 평면 메쉬 같이 시각화
            try:
                ...
                if plane_eq is not None:
                    ...
                    if np.linalg.norm(n) > 1e-6:
                        # 평면 위의 한 점 (origin) = -d * n / ||n||^2
                        origin = -d * n / (np.dot(n, n) + 1e-12)

                        # plane_transform: 이 평면이 z=0, normal=+z가 되도록 변환
                        T = plane_transform(origin, n)
                        scene.apply_transform(T)

                # 🔹 장면의 XY 크기에 맞춰 z=0 평면 메쉬 추가
                try:
                    ...
                    px = size_x * 1.2
                    py = size_y * 1.2
                    thickness = max(size_x, size_y) * 0.01  # 매우 얇은 두께

                    center_x = (min_b[0] + max_b[0]) / 2.0
                    center_y = (min_b[1] + max_b[1]) / 2.0

                    # z=0 평면 윗면이 z=0이 되도록, 중심을 살짝 아래로
                    ...
                    print(f"⚠️ 평면 메쉬 생성 중 경고: {e}")

                # 뷰어 띄우기 (별도 창)
                scene.show()

            except Exception as e:
                messagebox.showerror("시각화 오류", f"GLB 시각화 중 오류:\n{e}")
...
class App(TkinterDnD.Tk):
    def __init__(self):
        ...
        ttk.Label(self, text="썸네일 중 하나를 클릭해 바닥영역을 선택하세요", background="#222", foreground="white").pack()
    ...
    def run_reconstruction(self):
        if not self.images or self.seg_mask is None or self.selected is None:
            ...
            return
        # 🔹 세그한 이미지 경로(self.selected)도 같이 넘긴다

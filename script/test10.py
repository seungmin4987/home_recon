...
# ============================================================
# 가상 자(virtual ruler)를 3D용으로 그릴 때 박스로 만들기 위한 함수
# (Open3D에서는 별도 LineSet으로 그릴 것이므로 여기서는 사용 안 해도 됨)
# ============================================================
def add_virtual_ruler_trimesh(scene: trimesh.Scene,
                              p1: np.ndarray,
                              p2: np.ndarray,
                              thickness: float = 0.02,
                              color_rgba=None):
    """
    (이전 trimesh용 버전, 현재는 Open3D에서 LineSet으로 그릴 예정이라
     필요 없지만, 혹시 3D 팝업 등에 사용할 수 있어 남겨둠)
    """
    try:
...
class SnapshotFloorApp(tk.Tk):
    ...
    # 🔵 Open3D용 실시간 뷰 상태 빌드
    # ========================================================
    def _build_live_state(self):
        """
        Open3D 뷰어로 보낼 현재 상태를 dict로 구성.
        - room_points: (N,3) float64
        - room_colors: (N,3) float64 0~1
        - furnitures: [{wx, wy, yaw_rad, w, d, h}, ...]
        - ruler: {"p1": [x,y,z], "p2": [x,y,z]} or None
        """
        if self.scene_plane is None:
            return None

        # 방 포인트클라우드 + 바닥 포함
        pts, cols_rgba = scene_to_points_with_colors(self.scene_plane)
        if pts.size == 0:
            return None

        cols_rgb = cols_rgba[:, :3].astype(np.float64) / 255.0
        room_points = pts.astype(np.float64)
        room_colors = np.clip(cols_rgb, 0.0, 1.0)
        ...
        if self.ruler_enabled and self.ruler_world_p1 is not None and self.ruler_world_p2 is not None:
            p1 = self.ruler_world_p1.astype(float).tolist()
            p2 = self.ruler_world_p2.astype(float).tolist()
            ruler_state = {"p1": p1, "p2": p2}

        state = {
            "room_points": room_points,
            "room_colors": room_colors,
            "furnitures": furn_states,
            "ruler": ruler_state,
        }
        return state

    # ========================================================
    # 🔵 Open3D 팝업 3D 보기 (한 번만 띄우는 용도)
    # ========================================================
    def show_3d_popup(self):
        state = self._build_live_state()
        if state is None:
            messagebox.showwarning("알림", "3D로 표시할 데이터가 없습니다.")
            return

        geoms = self._build_o3d_geometries_from_state(state)
        if not geoms:
            messagebox.showwarning("알림", "3D로 표시할 geometry가 없습니다.")
            return

        vis = o3d.visualization.Visualizer()
        vis.create_window("3D 뷰 (Open3D)", width=960, height=720, left=0, top=0)
        for g in geoms:
            vis.add_geometry(g)
    ...
    # ========================================================
    # 🔵 Open3D용 geometry 생성 헬퍼
    # ========================================================
    def _build_o3d_geometries_from_state(self, state):
        geoms = []

        room_points = state.get("room_points")
        room_colors = state.get("room_colors")
        if room_points is not None and room_points.shape[0] > 0:
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(room_points)
            if room_colors is not None and room_colors.shape[0] == room_points.shape[0]:
                pcd.colors = o3d.utility.Vector3dVector(room_colors)
            geoms.append(pcd)

        # 가구들: footprint 박스를 Open3D box 메쉬로 생성
        furns = state.get("furnitures", [])
        for f in furns:
            w = max(f["w"], 1e-4)
            d = max(f["d"], 1e-4)
            h = max(f["h"], 1e-4)
            ...
            box = o3d.geometry.TriangleMesh.create_box(width=w, height=d, depth=h)
            # footprint 중심을 (0,0)에 두기
            box.translate([-w / 2.0, -d / 2.0, 0.0])
            R = o3d.geometry.get_rotation_matrix_from_xyz((0.0, 0.0, yaw))
            box.rotate(R, center=(0.0, 0.0, 0.0))
            box.translate([wx, wy, 0.0])
            box.compute_vertex_normals()
            box.paint_uniform_color([0.9, 0.7, 0.3])
            geoms.append(box)

        # 가상 자: LineSet
        ruler = state.get("ruler")
        if ruler is not None:
            p1 = np.array(ruler["p1"], dtype=float)
            p2 = np.array(ruler["p2"], dtype=float)
            ls = o3d.geometry.LineSet()
            ls.points = o3d.utility.Vector3dVector([p1, p2])
            ls.lines = o3d.utility.Vector2iVector([[0, 1]])
            ls.colors = o3d.utility.Vector3dVector([[1.0, 0.0, 0.0]])
            geoms.append(ls)

        return geoms

    # ========================================================
    # 🔵 Open3D 기반 실시간 3D 미리보기
    # ========================================================
        ...
        if self.live_preview_on:
            ...
            if self.live_preview_job is not None:
                try:
                    ...
                    pass
                self.live_preview_job = None
            self._send_live_view_command({"type": "close"})
    ...
    def refresh_live_preview(self):
        ...
        if state is None:
            return
        # Open3D 스레드에 상태 전달
        self._send_live_view_command({"type": "scene", "state": state})
    ...
    def _start_live_viewer_thread(self):
        ...
        def worker():
            ...
            try:
                ...
                running = True
                while running:
                    # 큐에서 최신 명령만 처리 (이전 것들은 버려도 됨)
                    try:
                        cmd = self.live_view_queue.get_nowait()
                        # 큐에 여러 개 있으면 마지막 것만 남기고 모두 버림
                        while True:
                            cmd_next = self.live_view_queue.get_nowait()
                            cmd = cmd_next
                        # 위 while은 queue.Empty가 날 때까지 돌다가 except로 빠짐
                    except queue.Empty:
                        cmd = None

                    if cmd is not None:
                        if isinstance(cmd, dict) and cmd.get("type") == "close":
                            running = False
                        elif isinstance(cmd, dict) and cmd.get("type") == "scene":
                            state = cmd.get("state")
                            geoms = self._build_o3d_geometries_from_state(state)
                            if geoms:
                ...
                print("[경고] Open3D 라이브 뷰어 스레드 오류:", e)
            finally:
                if vis is not None:
                    try:
                        vis.destroy_window()

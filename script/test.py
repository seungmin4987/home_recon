...
# ----------------- 천장 클리핑 파라미터 -----------------
# 바닥 평면에서 법선 방향으로 얼마나 올릴지 (단위: "진짜" 미터 기준 느낌)
# 예: 2.5m 위까지 남기고 그 위(천장/위쪽 벽)는 제거
CEILING_LIFT_DIST = 2.5  # 필요하면 값만 바꿔서 튜닝
...
def add_ceiling_plane(scene: trimesh.Scene, ceiling_height: float):
    ...
    - ceiling_height 는 z축 방향 거리 (예: 2.5m)
    """
    try:
        bmin, bmax = scene.bounds
        size = bmax - bmin
...
class SnapshotFloorApp(tk.Tk):
    ...
    def _load_and_align_scene(self):
        ...
        #    dist = (n·x + d) / ||n||
        #    → dist = 0 이면 바닥 평면, + 방향이 방 안쪽(천장 방향)이 되도록 orientation 자동 조정
        # -------------------------------
        a, b, c, d = plane_eq
        ...
        if isinstance(merged, trimesh.Trimesh) and merged.vertices.shape[0] > 0:
            ...
            if median_dist < 0.0:
                # 법선/평면 통째로 뒤집어서 "방 안쪽"이 +가 되게
                signed = -signed
                n = -n
                d = -d
                print("[INFO] plane orientation flipped so that inside is +dist")

            # 이제 dist=0 근처가 바닥, dist>0 쪽이 천장 방향
            # 👉 여기서부터가 "바닥 평면을 거리 H만큼 평행이동" 개념
            #    CEILING_LIFT_DIST 는 '진짜 거리(m)' 라고 가정하므로
            #    scene 단위로는 (CEILING_LIFT_DIST / metric_scale)
            if self.metric_scale <= 0:
                eff_lift = CEILING_LIFT_DIST
            else:
                eff_lift = CEILING_LIFT_DIST / self.metric_scale

            print(
                f"[DEBUG] metric_scale={self.metric_scale:.6f}, "
                f"CEILING_LIFT_DIST(real)={CEILING_LIFT_DIST:.3f}m, "
                f"eff_lift(scene)≈{eff_lift:.3f}"
            )

            # "바닥 평면을 eff_lift 만큼 올린" 평행 평면 아래(dist <= eff_lift)만 유지
            keep_mask = signed <= eff_lift + 1e-6
        ...
        # -------------------------------
        # 2) 평면 정렬 (plane_transform)
        #    ↳ 위에서 n, d가 이미 '방 안쪽이 +'가 되도록 뒤집힌 상태
        # -------------------------------
        a2, b2, c2, d2 = n[0], n[1], n[2], d
        ...
        # -------------------------------
        # 4) 바닥/천장 plane 시각화용 박스 추가
        #    - 바닥: z=0 근처
        #    - 천장: z=CEILING_LIFT_DIST (metric_scale 적용 이후라 '진짜 높이'와 매칭)
        # -------------------------------
        ...
        try:
            ...
            print(
                f"[DEBUG] aligned scene z-range≈[{z_min4:.3f}, {z_max4:.3f}], "
                f"ceil_plane_z≈{CEILING_LIFT_DIST:.3f}"
            )
        except Exception:
            pass

...
# GLB 병합 결과 캐시 (한 번만 로드/파싱)
_CACHED_BASE_MESH = None
_CACHED_PLANE_EQ = None

def _deepcopy_mesh(mesh: trimesh.Trimesh) -> trimesh.Trimesh:
    """
    trimesh.copy()가 내부 버퍼를 공유할 수 있어 스레드/겹침 시 색상이 깨질 수 있다.
    버텍스/페이스/컬러 버퍼를 명시적으로 복사한다.
    """
    m = mesh.copy()
    if hasattr(m, "vertices") and m.vertices is not None:
        m.vertices = np.array(m.vertices, copy=True)
    if hasattr(m, "faces") and m.faces is not None:
        m.faces = np.array(m.faces, copy=True)
    try:
        if hasattr(m, "visual"):
            if hasattr(m.visual, "vertex_colors") and m.visual.vertex_colors is not None:
                m.visual.vertex_colors = np.array(m.visual.vertex_colors, copy=True)
            if hasattr(m.visual, "face_colors") and m.visual.face_colors is not None:
                m.visual.face_colors = np.array(m.visual.face_colors, copy=True)
    except Exception:
        pass
    return m
...
def _get_cached_base_mesh():
    """
    GLB를 한 번만 로드/병합해 캐싱한 뒤 매 호출마다 copy()로 돌려준다.
    """
    global _CACHED_BASE_MESH, _CACHED_PLANE_EQ
    if _CACHED_BASE_MESH is not None and _CACHED_PLANE_EQ is not None:
        return _CACHED_BASE_MESH.copy(), _CACHED_PLANE_EQ

    glb_path, plane_eq = load_meta_and_glb()
    mesh_or_scene = trimesh.load(glb_path)
    merged = mesh_or_scene.dump(concatenate=True) if isinstance(mesh_or_scene, trimesh.Scene) else mesh_or_scene

    _CACHED_BASE_MESH = merged
    _CACHED_PLANE_EQ = tuple(plane_eq)
    return merged.copy(), _CACHED_PLANE_EQ


def _points_and_colors_from_mesh(mesh: trimesh.Trimesh):
    """
    dump 없이 Trimesh에서 vertex/색상을 바로 추출 (alpha 채워 넣기 포함).
    """
...
def add_virtual_ruler(scene: trimesh.Scene,
    ...
    z_offset < 0 이면 바닥 아래(뒤쪽)로 내릴 수 있다.
    """
    try:
        ...
        center[2] = center[2] + 0.3 + float(z_offset)
        #if color_rgba is None:
        #    color_rgba = np.array([255, 0, 0, 255], np.uint8)  # 빨간색
        ...
        # 기본 색상: 시인성 높은 마젠타
        if color_rgba is None:
            color_rgba = np.array([255, 0, 255, 255], dtype=np.uint8)
...
def align_scene_and_compute_ceiling(plane_flipped: bool = True):
    ...
    pts_aligned = trimesh.transform_points(pts_raw, T)

    # 3) 바닥을 z ≈ 0 으로 스냅 + 단일 quantile 패스
    auto_ceiling_height = None
    if pts_aligned.size > 0:
        q01, q95, q99 = np.quantile(pts_aligned[:, 2], [0.01, 0.95, 0.99])
        pts_aligned[:, 2] -= q01
        auto_ceiling_height = max(float(q95 - q01), 0.0)
        print(f"[INFO] floor snapped: ~{float(q01):.4f} → 0.0")
        print(f"[INFO] z-range≈[{0.0:.3f}, {float(q99 - q01):.3f}], auto ceiling(95%)≈{auto_ceiling_height:.3f}")
    else:
        print("[WARN] z_all 없음으로 quantile 계산 불가")
    ...
    if pts_aligned.size > 0:
        aligned_bounds = np.stack(
            [np.min(pts_aligned, axis=0), np.max(pts_aligned, axis=0)],
            axis=0
        )
    else:
        aligned_bounds = np.array([[0.0, 0.0, 0.0], [1.0, 1.0, 1.0]], dtype=float)

    scene = trimesh.Scene()
    scene.add_geometry(trimesh.points.PointCloud(pts_aligned, colors=cols_raw))

    # 디버깅용 최종 z-range
    try:
        z_min4, z_max4 = np.quantile(pts_aligned[:, 2], [0.01, 0.99])
        print(
            f"[DEBUG] aligned scene final z-range≈[{float(z_min4):.3f}, {float(z_max4):.3f}]"
        )
    except Exception:
        pass

    return scene, auto_ceiling_height, aligned_bounds, pts_aligned, cols_raw
...
class SnapshotFloorApp(tk.Tk):
    def __init__(self):
        ...
        # 오른쪽 GUI 폭을 화면의 45% (최소 800px)로 설정
        self.gui_w = max(int(self.screen_w * 0.45), 800)
        self.gui_h = self.screen_h
        # 좌/우 사이에 약간의 간격을 둠
        self.gap_px = 70
        # 왼쪽 3D 뷰어 폭은 나머지 영역에서 갭을 뺀 값
        self.viewer_w = max(self.screen_w - self.gui_w - self.gap_px, 800)
        self.viewer_h = self.screen_h
        # 메인 GUI를 오른쪽에 붙이되, 뷰어 오른쪽 끝에서 gap 만큼 떨어뜨림
        self.geometry(f"{self.gui_w}x{self.gui_h}+{self.viewer_w + self.gap_px}+0")
        # 평면 좌표계로 정렬된 씬 (방 전체) – **여기엔 가상자 안 넣는다**
    ...
    def _load_and_align_scene(self):
        ...
        6) 바닥 plane 추가
        👉 **여기에는 가상자 3D 메시는 넣지 않는다**
           (3D 뷰어용 가상자는 _build_scene_with_furniture 에서만 추가)
        """
        # 1) 정렬 + auto ceiling 계산 (여기서는 metric_scale 적용 전)
        scene_aligned, auto_ceiling_height, aligned_bounds, pts_aligned, cols_aligned = align_scene_and_compute_ceiling(
            self.plane_flipped
        )

        # 2) ⭐ 메트릭 스케일 재적용
        if self.metric_scale != 1.0:
            pts_aligned = pts_aligned * self.metric_scale
            aligned_bounds = aligned_bounds * self.metric_scale
        ...
        if num_keep == 0:
            ...
            pts_sel = pts_aligned
            cols_sel = cols_aligned
        else:
            pts_sel = pts_aligned[mask_keep]
            cols_sel = cols_aligned[mask_keep]

        # 5) 최종 Scene: 선택된 점(PointCloud) + 바닥 plane
        final_scene = trimesh.Scene()

        # 포인트가 가상자를 과도하게 가리지 않도록 alpha를 살짝 낮추고 점 크기를 줄인다.
        pc = trimesh.points.PointCloud(pts_sel, colors=cols_sel)
        pc.metadata["point_size"] = 2.0
    ...
    def _draw_ruler_overlay(self):
        ...
        self.ruler_line_id = self.canvas.create_line(
            px1, py1, px2, py2, fill="red", width=4
        )
    ...
    def _render_snapshot_and_set_camera(self):
        ...
        # 📸 스냅샷에도 가상자/가구가 보이도록, 가상자 포함 씬을 사용
        scene = self._build_scene_with_furniture()
        if scene is None:
            scene = self.scene_plane.copy()

        # 카메라 계산은 실제 렌더링에 들어갈 전체 씬(bounds 포함 가구/가상자) 기준
        try:
            bmin, bmax = scene.bounds
        except Exception:
        ...
        camera = trimesh.scene.cameras.Camera(resolution=(w, h),
        ...
        try:
            ...
            # 가상 자가 켜져 있고 포인트가 없으면 기본 위치 생성 후 2D 오버레이 갱신
            if self.ruler_enabled and (self.ruler_world_p1 is None or self.ruler_world_p2 is None):
                try:
                    bmin, bmax = self.scene_plane.bounds
    ...
    def toggle_ruler(self):
        ...
        - 3D 뷰어에는 _build_scene_with_furniture() 단계에서 3D 박스로 추가
        """
        ...
        # 2D 오버레이 갱신
        self._draw_ruler_overlay()
        # 3D 뷰어: 가상자 ON/OFF 시 현재 뷰어에 바로 반영
        if self.live_preview_on:
            scene = self._build_scene_with_furniture()
            if scene is not None:
                self._send_live_view_command(scene.copy())
        self._update_furniture_outline()
    ...
    def _build_scene_with_furniture(self):
        ...
        # 가상자가 켜져 있으면 포인트클라우드를 대폭 투명/작게 만들어 자가 가려지지 않게 한다.
        # 포인트 알파/크기 조정 없이 그대로 사용
        if self.templates:
        ...
        if self.ruler_enabled:
            ...
            # 기본 위치 없으면 최종 bounds로 생성
            if (self.ruler_world_p1 is None or self.ruler_world_p2 is None) and bmin_r is not None and bmax_r is not None:
                self._ensure_ruler_world_default((bmin_r, bmax_r))

            if self.ruler_world_p1 is not None and self.ruler_world_p2 is not None:
                z_offset_dyn = 1.0
                try:
                    if bmax_r is not None and np.all(np.isfinite(bmax_r)):
                        target_z = float(bmax_r[2])
                        avg_z = float(
                            (self.ruler_world_p1[2] if self.ruler_world_p1.shape[0] > 2 else 0.0)
                            + (self.ruler_world_p2[2] if self.ruler_world_p2.shape[0] > 2 else 0.0)
                        ) / 2.0
                        base_center = avg_z + 0.3
                        z_offset_dyn = 0.1#max(target_z - base_center, 1.0)
                except Exception:
                    pass
                add_virtual_ruler(
                    scene,
    ...
    def show_3d_popup(self):
        if self.scene_plane is None:
            return
        if self.live_preview_on:
            # 이미 라이브 뷰어가 켜져 있으면 새로운 창을 열지 않고 갱신만 한다
            scene = self._build_scene_with_furniture()
            if scene is not None:
                self._send_live_view_command(scene.copy())
            messagebox.showinfo("알림", "실시간 3D 미리보기가 켜져 있어 기존 창만 갱신했습니다.")
            return
        if not self.templates and not self.ruler_enabled:
            messagebox.showwarning("알림", "가구 템플릿 또는 가상자를 먼저 사용해보세요.")
            return
    ...
    def toggle_live_preview(self):
        # 창을 닫지 않고 보이지 않게만 처리
        turning_on = not self.live_preview_on
        ...
        self.live_preview_on = turning_on
        if turning_on:
            ...
            try:
                ...
                self.live_view_init_loc = None
            # 뷰어가 없으면 생성, 있으면 보이게
            if self.live_view_thread is None or not self.live_view_thread.is_alive():
                self._start_live_viewer_thread()
                self._schedule_live_preview_refresh(100)
            else:
                self._send_live_view_command("show")
                # 최신 씬 전송
                scene = self._build_scene_with_furniture()
                if scene is not None:
                    self._send_live_view_command(scene.copy())
            ...
            # 렌더 예약 취소 후 창을 숨김
            self._send_live_view_command("hide")

    def _close_live_viewer(self):
        """
        라이브 뷰어가 열려 있다면 안전하게 종료하고 상태를 리셋한다.
        """
        try:
            if self.live_view_queue is not None:
    ...
    def _restart_live_viewer(self):
        """
        뷰어를 확실히 닫았다가 다시 연다. 창은 항상 하나만 유지.
        """
        ...
        # 새 뷰어 스레드 시작 후 첫 씬 전송 예약
        self._start_live_viewer_thread()
        self._schedule_live_preview_refresh(100)

    def _reopen_live_viewer_with_scene(self, scene):
        """
        현재 뷰어를 닫고 다시 연 뒤 주어진 scene으로 초기화한다.
        """
        if scene is None or not self.live_preview_on:
            return
        # 대기 중 갱신 취소
        if self.live_preview_job is not None:
            try:
    ...
    def refresh_live_preview(self):
        ...
        # 뷰어가 닫혀 있거나 큐가 없으면 먼저 다시 열고 재시도 예약
        if self.live_view_thread is None or not self.live_view_thread.is_alive() or self.live_view_queue is None:
            self._start_live_viewer_thread()
            self.live_preview_job = self.after(100, self.refresh_live_preview)
            return
        ...
        def worker():
            ...
            try:
                ...
                if initial_scene is None:
                    return
                initial_scene = initial_scene.copy()
                viewer = trimesh.viewer.SceneViewer(
                    initial_scene,
                    start_loop=False,
                    caption="실시간 3D 뷰어",
                    resizable=True,
                    resolution=(self.viewer_w, self.viewer_h),
                )
                # 화면 왼쪽 절반에 고정 배치 (메인 GUI와 겹치지 않도록 동일 폭 사용)
                try:
                    try:
                        viewer.set_fullscreen(False)
                    except Exception:
                        pass
                    viewer_w = self.viewer_w
                    viewer_h = self.viewer_h
                    viewer.set_location(0, 0)
                    viewer.set_size(viewer_w, viewer_h)
                except Exception:
                    ...
                    try:
                        while True:
                            ...
                            if isinstance(cmd, trimesh.Scene):
                                ...
                                viewer.scene = viewer._scene = cmd.copy()
    ...
    def _send_live_view_command(self, cmd):
        if self.live_view_queue is None:
            return
        # Scene 또는 제어 명령("close","hide","show")만 허용
        if not (isinstance(cmd, trimesh.Scene) or cmd in ("close", "hide", "show")):
            return
        try:

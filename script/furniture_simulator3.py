...
class SnapshotFloorApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        size_frame.grid(row=7, column=0, sticky="ew", pady=(10, 5))
    ...
    def apply_furniture_size(self):
        """입력된 가로/세로(m)를 기준으로 target_w, target_d를 강제 조정"""
        if self.selected_index is None:
        ...
        # 현재 최종 크기 = target * scale
        # 우리가 원하는 것은 최종 크기가 w_val, d_val 이 되도록 target을 재조정
        ...
        messagebox.showinfo(
            "가구 크기 적용",
            f"선택 가구 크기를 가로 {w_val:.3f} m, 세로 {d_val:.3f} m 로 설정했습니다.\n"

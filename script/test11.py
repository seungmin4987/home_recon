...
class SnapshotFloorApp(tk.Tk):
    def __init__(self):
        ...
        self.measure_points_canvas = []
        self.measure_points_world = []
        self.measure_point_ids = []
        self.measure_line_id = None
        self.measure_real_dist_m = None

        self.metric_scale = 1.0

        self.live_preview_on = False
        self.live_preview_job = None
        self.live_preview_delay_ms = 300  # 디바운스 주기 약간 증가(부하 감소)
    ...
    def _start_live_viewer_thread(self):
        ...
        init_loc = self.live_view_init_loc
        ...
        def worker():
            ...
            try:
                ...
                try:
                    ...
                    viewer.set_size(max(int(screen_w * 0.5), 800), max(int(screen_h * 0.9), 700))
                except Exception:
                    if init_loc is not None:
                        try:
                            viewer.set_location(*init_loc)
                        except Exception:
                            pass

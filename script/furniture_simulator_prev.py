...
# ----------------- 천장 클리핑 파라미터 -----------------
# 바닥(z=0)에서부터 전체 높이(H)의 몇 %까지를 남길지
#   - 0.6 이면: 바닥 ~ 전체 높이의 60%까지만 살리고 그 위(천장/위쪽 벽)는 제거
CEILING_KEEP_FRAC = 0.6  # 0 < 값 < 1 사이로 두면 됨
...
def clip_scene_with_floor_plane(scene: trimesh.Scene,
                                keep_frac: float) -> trimesh.Scene:
    """
    🔥 바닥 평면(z=0)을 기준으로, 그와 평행한 평면을 위로 올려서
       그 평면 아래(z <= h) 에 있는 점들만 남기는 방식의 클리핑.

    - keep_frac: 0~1 사이 값
        0.6이면, 바닥에서 전체 높이의 60%까지만 남기고 위쪽은 제거.
    """
    ...
    try:
        ...
        z_all = merged.vertices[:, 2]
        if z_all.size == 0:
            return scene

        # 바닥(z=0) 근처와 천장 근처를 robust 하게 잡기 위한 퍼센타일
        z_floor = float(np.quantile(z_all, 0.01))  # 거의 0 근처
        z_ceil  = float(np.quantile(z_all, 0.99))  # 거의 천장 근처
        H = max(1e-6, z_ceil - z_floor)

        # 👉 바닥 평면을 z=0 이라고 보고, 그 평면을 keep_frac * H 만큼 위로 올린 것과 동일
        h_clip = z_floor + H * keep_frac

        # "법선을 유지한채로 평면을 들어올리고, 그 아래 점들만 남기기" = z <= h_clip
        keep_mask = merged.vertices[:, 2] <= h_clip + 1e-6
        if not np.any(keep_mask):
            print("[경고] 클리핑 결과가 비어, 원본 씬을 유지합니다.")
            return scene

        faces_keep = np.all(keep_mask[merged.faces], axis=1)
        if not np.any(faces_keep):
            print("[경고] 클리핑 결과가 비어, 원본 씬을 유지합니다.")
            return scene

        clipped_mesh = merged.submesh([faces_keep], append=True, repair=False)[0]
        new_scene = trimesh.Scene(clipped_mesh)
        print(f"[INFO] floor-plane clipping: keep_frac={keep_frac:.2f}, "
              f"z_floor≈{z_floor:.3f}, z_ceil≈{z_ceil:.3f}, h_clip≈{h_clip:.3f}")
        return new_scene
    except Exception as e:
        print("[경고] floor-plane 클리핑 실패:", e)
        return scene
...
class SnapshotFloorApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # 하단: 컨트롤 패널 (높이를 줄이기 위해 최소 padding)
        ...
        # 🔧 가구 크기 직접 지정 (m 단위 느낌, 실제 단위는 scene scale)
    ...
    #     # 원래 평면(ax+by+cz+d=0) 위의 한 점 p0
    #     p0 = -d * n / n_norm2

    #     # 👉 plane_transform(p0, n):
    #     #     - p0를 원점으로 옮기고
    #     #     - 법선 n을 +Z축(0,0,1)으로 맞춤
    ...
    #     try:
    #         bmin, bmax = scene.bounds
    #         z_floor = float(np.quantile(scene.dump(concatenate=True).vertices[:, 2], 0.01))
    #         scene.apply_translation([0.0, 0.0, -z_floor])
    #         print(f"[INFO] floor snapped: ~{z_floor:.4f} -> 0.0")
    #     except Exception as e:
    #         print("[경고] floor z 정렬 실패:", e)
    ...
    #     # 🔥 바닥 평면 기준으로 평행이동한 평면(z=h) 아래만 남기기 (천장 제거)
    #     scene = clip_scene_with_floor_plane(scene, CEILING_KEEP_FRAC)
    ...
    def _load_and_align_scene(self):
        ...
        merged = scene.dump(concatenate=True)
        if not isinstance(merged, trimesh.Trimesh):
            print("[WARN] merged mesh 가 Trimesh 가 아니라서 클리핑 생략.")
        else:
            verts = merged.vertices
            if verts.shape[0] > 0:
                # 원래 평면식 기준 signed distance
                ...
                if median_dist < 0.0:
                    orient = -1.0
                    signed = -signed  # 이제 바닥 ~0, 천장 방향이 + 로 향함

                # floor / ceil 거리 추정 (robust)
                floor_d = float(np.quantile(signed, 0.01))
                ceil_d = float(np.quantile(signed, 0.99))
                H = max(1e-6, ceil_d - floor_d)
                h_clip = floor_d + H * CEILING_KEEP_FRAC

                print(
                    f"[DEBUG] dist floor≈{floor_d:.3f}, ceil≈{ceil_d:.3f}, "
                    f"H≈{H:.3f}, h_clip≈{h_clip:.3f}, orient={orient:.1f}"
                )

                # lifted floor 평면 아래(dist <= h_clip)만 남김
                keep_mask = signed <= h_clip + 1e-6
                if not np.any(keep_mask):
                    print("[WARN] 클리핑 결과가 비어, 원본 scene 사용.")
                else:
                    faces_keep = np.all(keep_mask[merged.faces], axis=1)
                    if np.any(faces_keep):
                        clipped_mesh = merged.submesh(
                            [faces_keep], append=True, repair=False
                        )[0]
                        scene = trimesh.Scene(clipped_mesh)
                        print(
                            f"[INFO] floor-lift clipping 적용: "
                            f"keep_frac={CEILING_KEEP_FRAC:.2f}"
                        )
                    else:
                        print("[WARN] face 기준 클리핑 결과가 비어, 원본 scene 사용.")
            else:
                print("[WARN] verts 가 비어서 클리핑 생략.")

        # -------------------------------
        # 2) 평면 정렬 (plane_transform) - 클리핑 이후에 적용
        #    ↳ 위에서 orient 를 썼으므로, 평면 방향도 맞춰줌
        # -------------------------------
        # plane_eq * orient (법선 방향 맞추기)
        #  - orient는 위에서만 쓰고 여기서는 다시 계산 (median 기반)
        #  - 위가 None이면 그냥 raw 사용
        # 여기서는 한번 더 계산해서 일관성 유지
        merged2 = scene.dump(concatenate=True)
        if isinstance(merged2, trimesh.Trimesh) and merged2.vertices.shape[0] > 0:
            signed2 = (merged2.vertices @ n + d) / n_len
            median2 = float(np.median(signed2))
            orient2 = 1.0
            if median2 < 0.0:
                orient2 = -1.0
            print(f"[DEBUG] plane orient2={orient2:.1f} (for transform)")
        else:
            orient2 = 1.0

        a2, b2, c2, d2 = a * orient2, b * orient2, c * orient2, d * orient2

        # plane_flipped 토글은 여기에서만 적용 (클리핑에는 영향 X)
        if self.plane_flipped:
            a2, b2, c2, d2 = -a2, -b2, -c2, -d2
            print("[INFO] plane_flipped 적용됨.")
        ...
        try:
            ...
            z_all = merged3.vertices[:, 2]
        except Exception:
            z_all = None

        if z_all is not None and z_all.size > 0:
            z_floor3 = float(np.quantile(z_all, 0.01))
            scene.apply_translation([0.0, 0.0, -z_floor3])
            print(f"[INFO] floor snapped: ~{z_floor3:.4f} → 0.0")
        else:

...
# ----------------- 설정값 -----------------
# 천장 자동 계산 실패 시 사용하는 기본 높이 (fallback)
CEILING_LIFT_DIST = 2.5

# 평면 방향을 강제로 한번 더 뒤집고 싶을 때 사용하는 옵션
# (보통은 False로 두고, 필요하면 True로 바꿔서 테스트)
PLANE_FLIPPED = False
...
def add_ceiling_plane(scene: trimesh.Scene, ceiling_height: float, color_rgba=None):
    """
    z축 기준 ceiling_height 위치에 얇은 천장 plane 추가 (시각화용)
    color_rgba 를 지정하면 해당 색으로, 없으면 붉은 계열로 표시
    """
...
def add_sub_ceiling_plane(scene: trimesh.Scene, ceiling_height: float, ratio: float = 0.95):
    """
    천장보다 약간 아래 평면 추가.
    - ceiling_height 가 H라면, 서브천장 높이는 H * ratio (기본 0.95) 로 설정.
    - 즉, 전체 높이의 5% 만큼 아래로 내린 plane.
    """
    sub_height = ceiling_height * ratio
    print(f"[INFO] sub-ceiling height = {sub_height:.3f} (ratio={ratio})")

    # 서브천장은 구분을 위해 초록색 계열로 표시
    color = np.array([80, 200, 80, 130], np.uint8)
    add_ceiling_plane(scene, sub_height, color_rgba=color)
...
def build_scene_with_planes():
    ...
    4) 바닥 + 천장 + 천장보다 5% 낮은 서브천장 plane 시각화
    """
    ...
    try:
        ...
        if isinstance(merged, trimesh.Trimesh) and merged.vertices.shape[0] > 0:
            ...
            # 방 안쪽(천장 방향)이 + 가 되도록 자동으로 방향 맞추기
            if median_dist < 0.0:
                n = -n
                d = -d
    ...
    # 5) 바닥 + 천장 + 서브천장 plane 시각화용 박스 추가
    add_floor_plane(scene)

    if auto_ceiling_height is not None:
        # (1) 원래 천장 plane
        add_ceiling_plane(scene, auto_ceiling_height)
        # (2) 천장보다 약간 낮은 서브천장 plane (높이의 5% 아래)
        add_sub_ceiling_plane(scene, auto_ceiling_height, ratio=0.95)
    else:
        # auto 계산 실패 시, fallback 높이를 사용
        print("[WARN] auto ceiling 높이 계산에 실패하여 fallback 사용")
        add_ceiling_plane(scene, CEILING_LIFT_DIST)
        add_sub_ceiling_plane(scene, CEILING_LIFT_DIST, ratio=0.95)
...
if __name__ == "__main__":
    ...
    scene.show()  # trimesh 내장 뷰어 (pyglet 기반)
